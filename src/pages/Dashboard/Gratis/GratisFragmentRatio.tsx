...
const GratisFragmentRatio: React.FC = () => {
  const [fragmentData, setFragmentData] = useState<FragmentData[]>([]);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get<FragmentData[]>('http://localhost:5205/fragmentedindex?customer=DatabaseConnectionGratis');
...
export default GratisFragmentRatio;
